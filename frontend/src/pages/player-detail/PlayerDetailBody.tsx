...
interface PlayerDetailBodyProps {
  gsisId: string
  /** Present when reached from a native league's Roster/Players tab — lets
   *  the backend attach this player's contract in that league (a player's
   *  contract isn't a global attribute, it's scoped to one league's roster). */
  leagueId?: number
}

/**
 * The actual player-detail content (header, contract, grade, notes, season
 * stats, projection) with no page chrome of its own — shared by the full
 * `/players/:gsisId` route (`index.tsx`, which wraps this with a Back button
 * and page-width container) and `PlayerDetailPanel` (a side panel on desktop
 * / bottom sheet on mobile, opened in place from a roster/player table
 * instead of navigating away).
 */
export default function PlayerDetailBody({ gsisId, leagueId }: PlayerDetailBodyProps) {
