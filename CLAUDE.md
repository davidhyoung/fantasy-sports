...
- `src/pages/` — pages by route; complex pages split into subdirectories:
  ...
  - `draft-prep/` — `/draft-prep`: shared `DraftBoardTable` (also used read-only by league-detail's Draft tab) + a dockable `TeamPanel`/`TargetList` plan panel + `DraftSettingsPanel`
