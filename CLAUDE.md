...
- `internal/handlers/` — one file per resource; all handlers are methods on `Handler{db, sessions, oauthConfig, config}`
  ...
  - `draft_prep.go` — GetDraftPrep, UpsertDraftPrepPlayer, ReorderDraftPrep: the personal draft board in `draft_prep_players`, scoped to (user, league, season) — interest level (must/target/sleeper/avoid), custom rank, note, and planned cost
...
/draft-prep?view=board|team              Draft Prep — board (ranks, interest levels, notes) + team builder
/leagues                                → redirect to /
...
Protected (RequireAuth):
  ...
  PUT  /api/leagues/{id}/draft-prep/{gsisId}?season=     — {tag, custom_rank, note, planned_cost}; all-empty deletes the row
       tag ∈ must|target|sleeper|avoid (level of interest); planned_cost = what you intend to pay (null = not in the team plan)
...
- **Draft Prep page (`/draft-prep`):** the pre-draft surface. Owns everything editable about a draft board; the league page's Draft tab is the same board **read-only**, so there's one place to change things and no chance of the two disagreeing.
  ...
  - **Personal board** (`hooks/useDraftPrep.ts` → `draft_prep_players`): an ordered level of interest (**must** > **target** > **sleeper**, plus **avoid**), a custom ranking, a note, and a **planned cost** per player, scoped to (user, league, season). Server-side, not `localStorage` — this is the board you rely on during a live draft, so it has to survive a different browser. Writes are optimistic with rollback, since tagging players during prep should feel instant.
  ...
  - A row with no tag, no rank, no note **and no planned cost** carries no information, so that combination **deletes** the row (client and server agree on this).
