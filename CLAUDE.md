...
- **Draft Prep page (`/draft-prep`):** the pre-draft surface. Owns everything editable about a draft board; the league page's Draft tab is the same board **read-only**, so there's one place to change things and no chance of the two disagreeing.
  ...
  - **Bench (`BN`) is part of the slot vocabulary** but starts nobody — `ranking.ComputeStarterSlots` skips it, so it cannot move a replacement level or a price. It's carried because roster size is what tells the builder how many $1 spots a budget still has to cover.
