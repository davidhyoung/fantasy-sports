...
- `src/pages/` — pages by route; complex pages split into subdirectories:
  ...
  - `draft-prep/` — `index.tsx` (top-level `/draft-prep`, `?view=board|team`) + `components/` (DraftBoardTable, Shortlist, TeamBuilder) + `lib/roster.ts` + `hooks/useDraftPrep.ts`. **`DraftBoardTable` is the shared draft board**, used here with prep controls and by league-detail's DraftTab without them
...
/draft-prep?view=board|team              Draft Prep — board (ranks, interest scale −3..+3, notes) + team builder
/leagues                                → redirect to /
