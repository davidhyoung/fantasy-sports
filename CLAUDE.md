...
- **Draft Prep page (`/draft-prep`):** the pre-draft surface. Owns everything editable about a draft board; the league page's Draft tab is the same board **read-only**, so there's one place to change things and no chance of the two disagreeing.
  ...
  - **Personal board** (`hooks/useDraftPrep.ts` → `draft_prep_players`): a signed **interest scale** (+3 must draft, +2 love, +1 like, −1 dislike, −2 hate, −3 do not draft), a custom ranking, a note, and a **planned cost** per player, scoped to (user, league, season). The scale replaced a tag vocabulary (migration 000018) because it is *orderable* — "sort my board by how much I want him" is a column sort, not a lookup table. `NULL` is the only way to say "no opinion"; **0 is rejected** so there aren't two representations of it. Labels live in `lib/interest.ts`; the numbers are the source of truth. The board control is **three thumbs each way**, filling outward from the centre like a mirrored star rating — one thumb up is "like", three is "must draft" — so magnitude reads without a number on every button. Re-picking the current level clears it. Server-side, not `localStorage` — this is the board you rely on during a live draft, so it has to survive a different browser. Writes are optimistic with rollback, since tagging players during prep should feel instant.
