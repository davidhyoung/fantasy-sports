...
- `src/pages/` — pages by route; complex pages split into subdirectories:
  ...
  - `projections/` — `index.tsx` + `hooks/useProjections.ts` + `components/` (ProjectionTable, ConfidenceBadge, UniquenessBadge) — comp-based projection rankings with PPR/Half/Standard toggle; also shows Consensus/Δ columns (partial coverage, "—" where no consensus data exists)
  - `divergences/` — `index.tsx` + `hooks/useDivergences.ts` + `components/DeltaBadge.tsx` — standalone /divergences page: where our projections disagree most with expert consensus, within position group, with situational notes. Single-source divergences are flagged (`1*`) since they're uncorroborated
