...
- `internal/handlers/` — one file per resource; all handlers are methods on `Handler{db, sessions, oauthConfig, config}`
  ...
  - `league_rosters.go` — GetLeagueRosters, AssignLeagueRoster, UpdateLeagueRoster, DropLeagueRoster, GetLeagueFreeAgents: roster + contract management. Every roster row gets a contract row (even a $0 undrafted pickup) via the shared `assignRosterTx` helper (also used by `draft_picks.go`). `GetLeagueFreeAgents` is `nfl_players` minus `league_rosters`, ordered by projection, with an optional `?search=` filter. `validSlots` excludes `DEF`. Enforces lineup-slot eligibility (`slotEligibleForPosition`): QB/RB/WR/TE/K slots take only that position, FLEX takes RB/WR/TE/K, SFLEX takes anyone, BN/TAXI/IR take anyone. `DropLeagueRoster` writes dead money before cascading the roster/contract rows. Also home for `leagueTeamIDs`, `generateDraftClassTx`, and `relevantPlayerStats` (per-player stat totals restricted to whichever categories the league's own scoring weights nonzero — backs the dynamic stat columns on Roster/Players, plus a view-scoped `fpts` total). `relevantPlayerStats` takes a `statView` (`?view=week|season|rest|next4`, `&week=N` for `week`) parsed by `parseStatView`: `week`/`season` read real stats (`nflstats.LoadWeekStats`/`LoadSeasonStats`); `rest`/`next4` read `nfl_projections`' per-game rates scaled by however many games are actually left (`regular_season_weeks` minus `nflstats.LastCompletedWeek`, capped at 4 for `next4`) rather than the hardcoded 17-game default. `GetLeagueRosters`/`GetLeagueFreeAgents` both accept this param; free agents' own ordering (which players appear pre-pagination) stays fixed to `proj_fpts_ppr DESC` regardless of the selected view — only the displayed numbers change. Mutations commissioner-gated; reads RequireAuth only.
  ...
  - `scoring.go` — GetLeagueScoreboard, GetLeagueStandings: dispatch on `leagues.source` (`leagueSource()`) — Yahoo leagues hit the Yahoo API; native leagues read `league_matchups` directly (standings are a pure aggregation over scored matchup rows, no separate standings table). `nativeScoreboard` also calls `nativeLivePoints` — a best-effort ESPN-derived live preview per unscored matchup (`Status: "in_progress"`), per-team so one game's slate being live never bleeds into an unrelated matchup that hasn't kicked off. Purely a display estimate, same as `nativeProjectedPoints`; never written to `league_matchups`.
  - `schedule.go` — GenerateLeagueSchedule (round-robin "circle method", idempotent per season), ScoreLeagueWeek (an **explicit, re-runnable commissioner action, not automatic** — sums each team's starter-slotted players' real stat lines through `scoring.ScoreWithModifiers` and freezes the result onto `league_matchups`; underlying stats come from a manual `make import-nfl` batch import, never live). Also home for `starterRosters` (the shared "who's starting this week" query — current `league_rosters.slot` state, no per-week history — used by `ScoreLeagueWeek`, `nativeProjectedPoints`, and `nativeLivePoints`). Regular season only — no playoff bracket/seeding.
...
Optional:
- `LIVE_STATS_POLL=1` — start the ESPN live-stats poller (`internal/services/espnlive`). Off by default; safe to enable in production (unlike the entry below) since it's the flag that actually turns on the near-live scoreboard preview for native leagues. See "Live score preview" above.
...
- **Player detail:** `GET /api/nfl/players/{gsisId}` returns metadata + YoY stats + projection + `notes`. Every list-shaped endpoint that returns `gsis_id` batch-attaches notes via the shared `loadNotesForPlayers` helper. The notes button (`PlayerCell`, `table-helpers.tsx`) highlights when a note is "new" (`reported_date`/`created_at` within 3 days). **Scope cut:** desktop `PlayerCell` only — `MobileStatCard` doesn't show notes yet. `/players/:gsisId` (the full-page route) still exists for direct links, but every in-app player click app-wide opens `PlayerDetailPanel` in place instead of navigating there — a right-docked `SidePanel` on desktop, a `MobileSheet` below `md`, both rendering the same `PlayerDetailBody`. Every screen that lists players (`PlayerCell`/`MobileStatCard` rows, and the draft-prep family's own name buttons) owns a local `viewingPlayer` state (`useState<string | null>`) and an `onPlayerClick`/`onClick` callback wired to `PlayerCell`'s or `MobileStatCard`'s `onClick` prop (never their `href`), same convention `NativeRosterTable`/`NativeTeamOverview` established first. Pages with a league in scope (`PlayersTab`, `NativePlayersTab`, `MessagesTab`+`ThreadDetailView`, `draft-prep/index.tsx`, `league-detail/DraftTab.tsx`) pass `leagueId` through to the panel so its contract card/Edit-Drop-Trade footer can resolve; standalone pages (Home, Statistics, Projections, Divergences, matchup/team-detail rosters) omit it and just get the read-only body.
...
- **Native leagues:** a league with no Yahoo league behind it — `leagues.source = 'native'`, `leagues.format` selects the rollover strategy. `POST /api/leagues` creates one (settings + teams, one transaction), owned by `leagues.user_id`. `services/leaguesettings.Source` is the seam that lets `draft_values.go` (and everything downstream) work identically for Yahoo or native leagues. No `DEF` slot anywhere (no team-defense `gsis_id`s). Rosters/contracts live in `league_rosters`/`league_contracts`; future picks/trades in `league_draft_picks`/`league_transactions`; weekly play in `league_matchups`; salary cap in `league_team_seasons`/`league_dead_money`; free agency in `league_fa_windows`/`league_fa_offers`; message board in `league_posts` + related tables. Every mutation (assign/drop/trade/pick-use/rollover/sign) logs a `league_transactions` row, which backs the Roster tab's activity log and the message board's derived feed events. Team ownership is opt-in claiming (`PUT .../teams/{teamId}` `{claim: true}`, one team per user per league) rather than Yahoo-style sync-time assignment. There is no separate "team page" for a native team — every team-name link resolves into the league's Roster tab with `?team={id}`, since there's no reader who isn't the commissioner in this single-user-per-league model.
  - **Live score preview:** `internal/services/espnlive` polls ESPN's unofficial `site.api.espn.com` scoreboard/summary endpoints during NFL game windows (a `time.Ticker` goroutine started from `cmd/api` when `LIVE_STATS_POLL=1`, off by default) and upserts best-effort in-progress stats into `nfl_live_player_stats` (isolated table, keyed `(gsis_id, season, week)`, JSONB canonical-stat blob). `espnlive.LoadLiveWeekStats` mirrors `nflstats.LoadWeekStats`'s shape, so `nativeLivePoints` (scoring.go) scores it through the exact same `scoring.ScoreWithModifiers` pipeline as real/final stats. Player identity crosses over via `nfl_players.espn_id` (already populated from nflverse's own roster CSV — confirmed to match ESPN's athlete ids verbatim, no separate crosswalk import needed). Explicitly best-effort: the endpoints are undocumented and unauthenticated, this is a preview layer only, and it never writes to `nfl_player_stats`/`league_matchups` — official scoring is still the manual `make import-nfl` + `ScoreLeagueWeek` flow below.
  - **Explicit, deliberate scope cuts (not gaps to silently fill):** regular season only, no playoff bracket/seeding (`league_matchups.is_playoffs` reserved); *official* scoring is still a manual per-week commissioner action, never automatic (the ESPN live preview above is a display-only exception, never written anywhere); no native keeper-designation flow (`rollover.go`'s `keeper` format 501s); `analysis.go` rankings and `league_players.go` search are Yahoo-only, not yet ported to native; salary retention in trades is not implemented (the pressure valve that would make the full-dead-cap rule survivable long-term — see `.claude/plans/dynasty-transactions.md`'s "Open/deferred").
