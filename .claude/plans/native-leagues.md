...
- **Design-review pass implemented wholesale (2026-08-21).** Dave published a full design spec of the native-league feature as an Artifact, got back an 8-item design-review response (README + annotated HTML mockup, both themes) auditing it against the live design system, then said "do it all." Implemented across the board — full detail in `project_native_leagues.md`'s memory entry, headline summary here: a new `--format` CSS token + `Badge variant="format"` (green) replaces the format badge's purple misuse everywhere it shows (league header + Home list, which also gained a `⟳ Yahoo`/`Native` tag); `TeamAvatar` recolored to the same three-hue cycling convention as `LeagueMark` (was neutral gray); Standings gained an L5 win/loss strip + a size-heuristic playoff cutoff line + a "you" tint on the signed-in user's row; Scoreboard went two-column with real projected points on unscored matchups (new `nativeProjectedPoints` helper, one game's worth of projection rates through the league's own scoring) and a tinted winner treatment on scored ones; `NativeRosterTable` gained a merged Contract column (years remaining), a collapsed `⋯` actions menu, Starters/Bench/Taxi&IR grouping, a drag-handle glyph, and — flagged by the review as the single highest-priority fix — accessible per-row "place here"/"swap here" labels (+ `aria-label`/`role="button"`) on the picking-mode banner, since color/cursor alone don't exist for a screen reader; Players tab got multi-select position filters, its Free Agents/Rostered toggle converted from chips to a tray (per the review's now-codified rule: tray switches dataset, chips filter within one), and a new L4-weeks sparkline off a new `weeklyTrend` backend helper; the Trade dialog gained a "Review trade →" summary step before executing (no real undo exists for a trade). Two deliberate deviations from the review: kept the Roster's Eligible column showing the full eligible-slot list rather than truncating to bare position (the review's own document contradicted itself on this, and truncating was explicitly flagged as a possible UX regression); didn't build the Players tab's proposed Season Total/Per Game/L4/L8 time-window dropdown (no backend support for non-season aggregation windows exists — shipped only the always-real L4 sparkline instead of a control that would partly be inert). Verified live end-to-end on league 42; `go build`/`go vet`/`go test ./...` and `tsc --noEmit`/`yarn build` all clean.
- **Roster moved to second tab (2026-08-21).** Nav order for native leagues is now Standings, Roster, Scoreboard, Players, Draft. Yahoo unaffected.
- **Team avatars, placeholder for now (2026-08-21).** `TeamAvatar` (`components/ui/table-helpers.tsx`) renders a flat initials circle per team — no real logo pipeline exists, so this is a stand-in matching `PlayerAvatar`'s no-image fallback styling. Default-seeded teams are all "Team N"; naive initials would collapse "Team 1"/"Team 10" to the same "T1", so `teamInitials()` special-cases that pattern to show the full number instead. Wired into Standings, Scoreboard matchup cards, the Roster tab's matchup card (both sides), and next to the team-switcher dropdown. Verified live across all three tabs and all 12 teams.
- **Picking button moved to Slot, Pos lists eligible slots (2026-08-21).** The picking-mode trigger moved from the Pos cell to the Slot cell (same button chrome, just relocated); Pos is plain text again, now showing every starter-eligible slot for the player (e.g. "RB · FLEX · SFLEX"), excluding the universal BN/TAXI/IR. Verified live.
