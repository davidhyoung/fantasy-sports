...
- **Not started:** `analysis.go` native rankings (still Yahoo-only — the category-metadata shape doesn't map 1:1 onto the native league's narrower editable-scoring vocabulary), `league_players.go` native player search (no equivalent to Yahoo's player search exists for native leagues — `GetLeagueFreeAgents`'s `?search=` covers the free-agent half only), native keeper-format rollover, draft order / `overall_pick` (no standings exist for native leagues, so picks are generated in team-id order and never get a real draft slot).
